...
"・相手が追加で話し出しても、短く受け止めて締め直す\n"
        ...
        self.likelihoods = {
            ...
            "H1": {
                ...
                ActionType.MINIMAL: 0.15,      # 「はい/そうですね」等の最小応答
                ActionType.NORMAL: 0.20,       # 内容のある通常返答
                ActionType.DISCLOSURE: 0.55,
            },
            # H0 = 話したくない時に出やすい観測（合計=1）
            "H0": {
                ActionType.SILENCE: 0.50,
                ActionType.MINIMAL: 0.30,
                ActionType.NORMAL: 0.15,
    ...
    def classify_action(self, user_text: Optional[str]) -> ActionType:
        ...
        4クラス分類：
          - SILENCE: 発話なし
          - MINIMAL: 「はい/そうですね/ありがとうございます」など、会話が広がりにくい最小応答
          - NORMAL: 現在状況の一言、事実のみ、短い返答（ただしMINIMALほど短くない）
          - DISCLOSURE: 過去体験・好み/価値観・感情・具体的エピソード等を含む自己開示
        """
        if user_text is None:
            return ActionType.SILENCE

        # まずは最小応答を優先して検出（LLMに回す前に確定させる）
        if self._is_minimal_reply(user_text):
            return ActionType.MINIMAL

        # 速さ優先：短文で相づち系は NORMAL に寄せる
        if len(user_text) <= 8 and any(w in user_text for w in ["はい", "うん", "そう", "ありがとう", "ええ", "なるほど"]):
        ...
        prompt = (
            ...
            "・NORMAL: 事実のみ/現在状況の一言/短い返答で、自己開示が含まれない。\n"
