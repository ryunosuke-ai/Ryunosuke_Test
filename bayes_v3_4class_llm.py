...
"・相手が追加で話し出しても、短く受け止めて締め直す\n"
        ...
        self.likelihoods = {
            # H1 = 話したい時に出やすい観測（合計=1）
            "H1": {
                ActionType.SILENCE: 0.05,
                ActionType.MINIMAL: 0.10,      # 「はい/そうですね」等の最小応答
                ActionType.NORMAL: 0.30,       # 内容のある通常返答
                ActionType.DISCLOSURE: 0.55,
            },
            # H0 = 話したくない時に出やすい観測（合計=1）
            "H0": {
                ActionType.SILENCE: 0.50,
                ActionType.MINIMAL: 0.35,
                ActionType.NORMAL: 0.10,
        ...
        self.max_total_turns: int = 30  # 暴走防止（必要なら調整）
    ...
    def classify_action(self, user_text: Optional[str]) -> ActionType:
        """
        4クラス分類：
          - SILENCE: 発話なし（STT結果が None）
          - MINIMAL: 反応はあるが情報が少なく、会話を広げにくい最小応答/相づち/低関与
          - NORMAL: 事実/現在状況/簡単な回答など「応答として成立」しているが、自己開示は薄い
          - DISCLOSURE: 過去体験・好み/価値観・感情・理由・具体エピソード等を含む自己開示

        方針：
          - SILENCE はルールで確定
          - それ以外（MINIMAL/NORMAL/DISCLOSURE）は LLM に 3クラス分類させる
            ※ ルールベースに寄せず、会話文脈に依存するニュアンス（例：「別に」「そうなんだ」など）も
               MINIMAL として判定できるようにする。
        """
        ...
        prompt = (
            "あなたは会話ログの分類器です。次のユーザー発言を、次の3つのいずれかに分類してください。\n"
            "\n"
            "【分類ラベル】\n"
            "MINIMAL / NORMAL / DISCLOSURE\n"
            "\n"
            "【定義】\n"
            "MINIMAL: 相づち/短い受け止め/低関与。新しい情報がほぼない、話題が広がりにくい。"
            "例：『はい』『そうですね』『へー』『ふーん』『そうなんだ』『別に』『うーん』など。"
            "（短くても、情報や理由が入っていれば MINIMAL ではないことがある）\n"
            "NORMAL: 応答として成立しており、事実・現在状況・簡単な説明など最低限の情報はある。"
            "ただし、過去の体験・好み・感情・価値観・理由・個人エピソードなどの自己開示は薄い。"
            "例：『今日は家で休みます』『特に予定はないです』『今は部屋にいます』など。\n"
            "DISCLOSURE: 自己開示が含まれる。過去の体験（昔〜した）、好み/価値観（好き/苦手/こだわり）、"
            "感情（嬉しい/つらい）、理由・背景、具体的エピソード（いつ/どこで/誰と/何があった）など。\n"
            "\n"
            "【出力形式】\n"
            "MINIMAL / NORMAL / DISCLOSURE のいずれか1語だけ（他の文字は出力しない）。"
        )
        ...
        try:
            ...
            if out.startswith("MINIMAL"):
                return ActionType.MINIMAL
            if out.startswith("DISCLOSURE"):
                return ActionType.DISCLOSURE
            return ActionType.NORMAL

        except Exception as e:
            self.logger.warning("分類LLM失敗: %s", e)
            # 失敗時は安全側：会話を壊しにくい NORMAL
            return ActionType.NORMAL
    ...
    def transition_policy(self, obs: Observation) -> None:
        ...
        if self.phase == Phase.BRIDGE:
            ...
            if obs.action_type in [ActionType.SILENCE, ActionType.MINIMAL] or self.p_want_talk < 0.35:
        ...
        if self.phase == Phase.DEEP_DIVE:
            ...
            if obs.action_type == ActionType.DISCLOSURE:
                ...
                return
            if obs.action_type in [ActionType.MINIMAL, ActionType.NORMAL, ActionType.SILENCE]:
    ...
    def update_conv_memory(self, user_text: Optional[str]) -> None:
        ...
        prompt = (
            ...
            "出力：JSONのみ。\n"
            "必ず次のキーを含める：\n"
            "  summary: これまでの会話の要約（日本語、350文字以内、箇条書き可）\n"
            "  do_not_ask: すでにユーザーが答えた/確定した内容で、AIが繰り返し質問すべきでない項目の短いリスト（最大8件）\n"
            "  stop_intent: ユーザーが会話を終えたい意思があるなら true（今回の発言から判断）\n"
            "\n"
            "注意：do_not_ask は『質問文』ではなく『項目名』にする（例：今日の予定、好きなジャンル、好きな作家、好きな作品）。\n"
        )
...
# -----------------------------
    ...
    def run(self) -> None:
        self._banner("会話ロボット")
        # SETUP相当の一言（固定）
        self.speak("こんにちは。たくさんお話しできたら嬉しいです！")
