...
"・相手が追加で話し出しても、短く受け止めて締め直す\n"
        ...
        # 尤度（更新係数）
        # 教授メモの「尤度×事前確率で更新（事後＝新しい事前）」に合わせて、
        # 観測ごとに p_want_talk を“倍率”で増減させる。
        # ※これは厳密なベイズ更新（正規化あり）ではなく、スコア更新に近い。
        #   値は実験ログを見ながら調整してください。
        self.likelihood_factors = {
            ActionType.SILENCE: 0.70,      # 沈黙：話したい度を下げる
            ActionType.NORMAL: 0.90,       # 生返事：少し下げる
            ActionType.DISCLOSURE: 1.30,   # 自己開示：上げる（>1 を許容）
        }
    ...
    def update_posterior(self, action_type: ActionType) -> float:
        """\
        教授メモに合わせた更新（事後 = 尤度 × 事前）。

        - ここでの「尤度」は確率 P(観測|状態) ではなく、
          事前確率を増減させる *倍率（係数）* として扱う。
        - そのため 1 を超える値（例：1.3）も許容する。
        - ただし p_want_talk は確率として扱いたいので、最後に [0.001, 0.999] にクリップする。

        ※ 厳密なベイズ更新（分母で正規化）にしたい場合は、bayes_v3.py の方式が適切。
        """
        prior = float(self.p_want_talk)
        factor = float(self.likelihood_factors.get(action_type, 1.0))

        posterior = prior * factor
        self.p_want_talk = max(0.001, min(0.999, posterior))

        print(
            f"📊 P(話したい) 事前 {prior:.2f} → 観測 {action_type.value} "
            f"→ 係数× {factor:.2f} → 事後 {self.p_want_talk:.2f}"
        )
...
# -----------------------------
    ...
    def run(self) -> None:
        ...
        # SETUP相当の一言（固定）
        self.speak("こんにちは。周りを一緒に見ながら、思い出話につながる会話ができたら嬉しいです。無理のない範囲で大丈夫ですよ。")
